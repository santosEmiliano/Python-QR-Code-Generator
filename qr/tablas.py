"""Consulta pura de las tablas fijas del estandar ISO/IEC 18004.

    - Tabla 2  : indicadores de modo
    - Tabla 3  : bits del indicador de cantidad de caracteres
    - Tabla 7  : capacidad de datos por version y nivel
    - Tabla 9  : caracteristicas de correccion de errores (bloques)
    - Tabla 12 : indicadores de nivel de correccion (para la info de formato)
    - Tabla 1  : bits de relleno finales (remainder bits)
    - Anexo E  : posiciones de los patrones de alineacion
...
# Los 45 caracteres del modo alfanumerico. La posicion en esta cadena es el
# valor numerico del caracter (0..44). ISO/IEC 18004, Tabla 5.
CARACTERES_ALFANUMERICOS = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ $%*+-./:"

# Indicador de modo: 4 bits que van al principio del mensaje. Tabla 2.
...
# Indicador de nivel de correccion: 2 bits que van en la info de formato.
# OJO: el orden NO es alfabetico. M es 00, no L. Tabla 12.
...
# Bits del indicador de cantidad de caracteres, segun modo y tramo de version.
# El tramo cambia en los cortes v1-9 / v10-26 / v27-40. Tabla 3.
...
# Bits de relleno finales: se agregan sueltos, al final del
# todo, porque la zona de datos no siempre es multiplo de 8. Tabla 1.
...
# Cantidad total de bytes (datos + correccion) por version. Tabla 9, columna
...
# Estructura de bloques de correccion. Tabla 9.
#
...
# Coordenadas (fila = columna) de los centros de los patrones de alineacion.
...
# Version mas alta con tablas por-version cargadas.
_VERSION_MAX_CARGADA = 10

# Helpers privados

def _validar_version(version: int) -> None:
    """Una version valida del estandar es un entero de 1 a 40."""
...
def _exigir_cargada(version: int) -> None:
    """Corta con un mensaje claro si se pide una version todavia sin transcribir."""
    _validar_version(version)
    if version > _VERSION_MAX_CARGADA:
        raise NotImplementedError(
            f"las tablas por-version solo estan cargadas hasta la v{_VERSION_MAX_CARGADA}; "
...
def bits_cuenta_caracteres(version: int, modo: Modo) -> int:
    """Cuantos bits ocupa el numero "cantidad de caracteres" dentro del mensaje.
...
def capacidad_bytes_datos(version: int, nivel_correccion: NivelCorreccion) -> int:
    ...
    nivel de correccion. Es lo que P1 tiene que llenar exacto con datos + relleno.
    """
...
def bloques_correccion(
    ...
    Devuelve una lista de tuplas  (cuantos_bloques, bytes_totales_por_bloque,
    bytes_de_datos_por_bloque), en el orden en que los define el estandar.
...
def posiciones_alineacion(version: int) -> list[int]:
    """Coordenadas de los centros de los patrones de alineacion (fila = columna).
...
def bits_relleno_final(version: int) -> int:
    ...
    Existen porque la zona de datos del QR no siempre es multiplo de 8.
    """
    _validar_version(version)
...
def indicador_modo(modo: Modo) -> list[int]:
    """Los 4 bits que definen el tipo numerico / alfanumerico / byte. MSB-first."""
...
def indicador_nivel(nivel_correccion: NivelCorreccion) -> list[int]:
    """Los 2 bits del nivel de correccion, para la etiqueta de formato."""
    if nivel_correccion not in _INDICADOR_NIVEL:
        raise ParametroInvalido(f"nivel de correccion desconocido: {nivel_correccion!r}")
